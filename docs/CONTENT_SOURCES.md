...
共同边界：用户已明确这两个项目属于广州逐电科技有限公司实习经历，因此可以归入实习项目。代码可以证明项目具备某项能力，但不能单独证明个人负责比例、独立完成范围、上线规模、业务收益或实习起止日期。未经进一步确认，使用“参与开发”描述个人经历，不使用“独立负责”“主导架构”等表述，不添加未经验证的性能、用户量或提升比例。
...
> 参与船舶安全告警 AI 复检平台开发，将模型推理、人工审核与告警处理串联成可追溯业务闭环。项目涵盖 Redis 负载调度、SSE 任务协同与配置化审核流程。
...
> 参与无感考勤系统开发，打通人员资料、人脸信息与抓拍事件的跨平台同步。通过任务状态流转、失败重试入口和上报阈值配置，让设备侧事件进入可管理的业务流程。
...
面向客船与危险品船运营企业的岸基智能监控平台，连接船端数据接入、位置上报、告警证据、告警处置及视频工作台。可用简介：“参与船舶智能监控云平台开发，串联终端接入、告警证据、组织权限与历史视频，让船端数据进入岸基业务流程。”
...
- 用户明确本项目属于广州逐电科技有限公司实习经历，可用“实习参与项目”；仓库不能单独证明个人独立完成范围。
...
部署于船端或边缘侧，接收 AI 抓拍结果，通过配置化规则生成正式告警，再经 JT808 上报并关联回执。可用简介：“参与船端抓拍告警分析系统开发，围绕规则策略、滑动窗口、冷却控制和上报回执，将识别结果转为可处置的告警。”
...
- 用户明确本项目属于广州逐电科技有限公司实习经历。网站使用“参与开发”，不根据代码推断个人独立负责整个系统。
...
- 未确认可公开访问且属于用户的 GitHub 仓库 URL，不编造链接；页面可标注“GitHub 项目”而不提供未经验证的外链。
